...
def img_to_ubyte_array(img):
    ...
    for img will also work. Passing a raw pillow image type should now also work
    """
    if type(img) == PIL.Image.Image:
        ret_ = skimage.util.img_as_ubyte(np.array(img))
    else:
...
def compute_segmentations(
    shapes = None,
    img_path=None, #string or pillow image
    features=None,
    shape_layers=None,
    label_to_colors_args={},
    clf = None # pass previous classifier instead of training a new one
):
    #if a classifier hasn't been passed, then make one
    if clf is None:
    

        # load original image or image passed
        ...
        t2 = time()
        print(t2 - t1)
    else: #if user has provided a clf to use (TODO add check to make sure the labels are consistant)
        print('user provided')
        t1 = time()
